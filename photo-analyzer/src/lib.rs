...
pub fn new_var_store() -> VarStore {
    VarStore::new(tch::Device::Cpu)
}

pub fn load_weights(vs: &mut VarStore, weights_fname: &Path) -> Result<Box<dyn ModuleT>, String> {
    // Create the model and load the weights from the file.
    let net: Box<dyn ModuleT> = match weights_fname.file_name().unwrap().to_str().unwrap() {
        ...
        "inception-v3.ot" => {
            println!("Loading inception");
            Box::new(inception::v3(&vs.root(), imagenet::CLASS_COUNT))}
        ,
        ...
        missing_model => {
            return Err(format!(
                "unknown model '{}', use a weight file named e.g. resnet18.ot",
                missing_model
            ))
        }
    };
    vs.load(weights_fname)
        .map_err(|err| format!("Failed loading weights: {:?}", err))?;
    Ok(net)
}
...
pub fn classify_image(
    ...
    let image = imagenet::load_image_and_resize224(image_path)
        .map_err(|err| format!("Failed loading image {:?} due to {}", image_path, err))?;
    ...
    Ok(imagenet::top(&output, 5)
        .iter()
        .map(|(probability, class)| Classification {
            class: class.into(),
